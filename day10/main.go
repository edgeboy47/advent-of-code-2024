...
func trailHeadRating(maps [][]int, row, col int) int {
	maxRows := len(maps) - 1
	maxCols := len(maps[0]) - 1
	...
	if row > 0 {
		...
		if up == curr+1 {
			score += trailHeadRating(maps, row-1, col)
		}
	}
	...
	if col > 0 {
		...
		if left == curr+1 {
			score += trailHeadRating(maps, row, col-1)
		}
	...
	for row := range topoMap {
		for col := range topoMap[row] {
			if topoMap[row][col] == 0 {
				...
				ratings += trailHeadRating(topoMap, row, col)
			}
		}
	}

	fmt.Printf("scores: %v", scores)
	fmt.Printf("ratings: %d", ratings)
}
